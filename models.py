...
class BaseModel:
    ...
    def compile(self):
        """Compiles the model."""
        self.model.compile(optimizer=optimizers.Adam(learning_rate=self.conf.l_rate),
    ...
    def fit(self, train_ds, val_ds, extra_callbacks=[]):
        ...
        checkpoint = callbacks.ModelCheckpoint(filepath=self.conf.exp_dir, save_weights_only=True,
                                               verbose=0)
        ...
        stopping = callbacks.EarlyStopping(monitor=self.es_monitor, min_delta=self.conf.es_delta,
                                           patience=self.conf.es_epochs, verbose=1, mode='min')
        ...
        self.history = self.model.fit(train_ds, epochs=self.conf.train_epochs, verbose=1,
...
class SingleParModel(BaseModel):
    ...
    def build(self):
        ...
        x = Conv2D(self.conf.filters, self.conf.kernel_size, padding='same', activation='relu')(inputs)
        x = Conv2D(self.conf.filters, self.conf.kernel_size, activation='relu')(x)
        x = MaxPooling2D(pool_size=2)(x)
        x = Dropout(self.conf.dropout)(x)
        x = Conv2D((self.conf.filters*2), self.conf.kernel_size, padding='same', activation='relu')(x)
        x = Conv2D((self.conf.filters*2), self.conf.kernel_size, activation='relu')(x)
        x = MaxPooling2D(pool_size=2)(x)
        x = Dropout(self.conf.dropout)(x)
        x = Flatten()(x)
