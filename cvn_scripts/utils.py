...
class DataHandler:
    ...
    def load_data(self):
        ...
        num_training = int(total_events*(1-self.val_frac-self.test_frac))
        # Number to use for validation
        num_validate = int(total_events*self.val_frac)

        # Split into training, validation and test samples
        split_data = np.split(
            data, [num_training, num_training+num_validate, total_events])

        # Decode the split data samples into separate np arrays
        train_labels, train_pars, train_hit_images, train_time_images = self.decode(
            split_data[0])
        val_labels, val_pars, val_hit_images, val_time_images = self.decode(
            split_data[1])
        test_labels, test_pars, test_hit_images, test_time_images = self.decode(
            split_data[2])
